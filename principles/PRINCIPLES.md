...
5. **What a machine can check never reaches a reader.** The plan is born ready and proved mechanically — citations by content, scope paths, acceptance numbering, declared corrections, and whole-file reading proved from the transcript, never a sentence saying so; a gap that needs new policy, fences or sections is escalated, not built. Mechanism: `plan-preflight.sh` through `plan-review-gate` (`plan_gate`), rounds, ESCALATE, growth guard.
