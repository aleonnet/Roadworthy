...
# size is printed so a gate can pin it.
# Usage: refute-ledger.sh <test dir> [--marker <regex>] [--record <regex>] [--legacy <file>]
set -euo pipefail
dir="${1:?usage: refute-ledger.sh <test dir> [--marker re] [--record re] [--legacy file]}"; shift
marker='FENCE|GUARD|CERCA'; record='refut|inject|Actual:'; legacy=""
while [ $# -gt 0 ]; do
  case "$1" in
