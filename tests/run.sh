...
# The local evidence ledger and the recorded state are not plugin sources: close.sh writes a
# gate's own output there, which carries the absolute paths of the machine that ran it. They are
# gitignored, and excluded here for the same reason tree-fingerprint.sh leaves them out of the
# fingerprint. Everything else, `.roadworthy/scope` and `.roadworthy/gates` included, is scanned.
LEDGERS='^\./\.roadworthy/(evidence\.jsonl|denials\.jsonl|state)'
...
# The ledger must stay out of git: committing it publishes those paths.
if git ls-files --error-unmatch .roadworthy/evidence.jsonl .roadworthy/state >/dev/null 2>&1; then
  fail "the local evidence ledger is tracked by git; it carries machine paths"
else ok "the evidence ledger is not tracked"; fi
