...
for f in hooks/lib.sh hooks/principles hooks/protect-paths hooks/scope-lock hooks/guard-commit hooks/plan-review-gate hooks/overnight-guard hooks/rite-gate hooks/stop-gate skills/*/scripts/*.sh tests/run.sh tests/attack.sh; do
  bash -n "$f" && ok "bash -n $f" || fail "bash -n $f"
done
if command -v shellcheck >/dev/null 2>&1; then
  shellcheck -S warning -x hooks/lib.sh hooks/principles hooks/protect-paths hooks/scope-lock hooks/guard-commit hooks/plan-review-gate hooks/overnight-guard hooks/rite-gate hooks/stop-gate skills/*/scripts/*.sh tests/run.sh tests/attack.sh \
...
# ── the cheating suite ──────────────────────────────────────────────────
# Run from here so CI carries it with no second job, and folded in as one line: attack.sh names
# every attack in its own output, and the whole point of it is the ones that are DECLARED.
section "attack.sh (the cheating suite)"
if bash "$ROOT/tests/attack.sh" > "$TMP/attack.out" 2>&1; then
  ok "$(tail -1 "$TMP/attack.out")"
else
  fail "an attack got through undeclared"; sed 's/^/      /' "$TMP/attack.out" >&2
fi
